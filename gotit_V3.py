...
def buscar_palavra_em_arquivos(diretorio, palavra):
    """
    Busca uma palavra em arquivos .prpt, .kjb e .ktr de forma recursiva.
    Retorna uma lista com os caminhos dos arquivos que contêm a palavra.
    """
    resultados = []
    ...
    # Extensões de arquivo que serão verificadas
    extensoes = ('.prpt', '.kjb', '.ktr')
...
def opcao_buscar_palavra():
    """Opção para buscar uma palavra em arquivos do diretório"""
    palavra = input("Digite a palavra que deseja buscar: ").strip()
    if not palavra:
        print("[ERRO] Palavra não pode estar vazia.")
        return
    
    diretorio = input("Digite o caminho do diretório para pesquisa: ").strip()
    if not os.path.isdir(diretorio):
        print("[ERRO] Diretório inválido.")
        return
    
    print(f"\n🔍 Buscando por '{palavra}' em {diretorio}...")
    resultados = buscar_palavra_em_arquivos(diretorio, palavra)
    
    if resultados:
        print(f"\n✅ Foram encontrados {len(resultados)} arquivos com a palavra '{palavra}':\n")
        for i, caminho in enumerate(resultados, 1):
            print(f"{i}. {caminho}")
    else:
        print(f"\n❌ Nenhum arquivo encontrado com a palavra '{palavra}'.")
...
#            #f.write.lower(conteudo)
#            f.write(conteudo)
#    except:
#        pass  # Ignora arquivos que não podem ser lidos como texto

def substituir_em_arquivo(caminho_arquivo, substituicoes):
    """Aplica substituições em um único arquivo de texto (sem distinguir maiúsculas/minúsculas)."""
    try:
        with open(caminho_arquivo, 'r', encoding='utf-8') as f:
            conteudo = f.read()
        for antiga, nova in substituicoes:
            # Substitui ignorando maiúsculas/minúsculas
            conteudo = re.sub(re.escape(antiga), nova, conteudo, flags=re.IGNORECASE)
        with open(caminho_arquivo, 'w', encoding='utf-8') as f:
            f.write(conteudo)
    except Exception as e:
        print(f"[ERRO] Não foi possível processar o arquivo {caminho_arquivo}: {e}")
...
def menu_principal():
    ...
    while True:
        ...
        print("5. BUSCAR palavra em arquivos (.prpt, .kjb, .ktr)")
        print("6. Sair")
